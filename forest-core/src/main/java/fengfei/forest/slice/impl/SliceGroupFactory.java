    ...
    private GroupConfig inherit(GroupConfig group) throws CloneNotSupportedException {
        ...
        if (parentId == null) {
            ...
            if (parentGroup == null) {
                ...
                config.setFuncType(group.getFuncType() == null ? config.getFuncType() : group
                    .getFuncType());
                config.setNavigationType(group.getNavigationTypeString() == null ? config
                    .getNavigationTypeString() : group.getNavigationTypeString());
                config.setOver(group.getOver() == null ? config.getOver() : group.getOver());
                config.setPlotterClass(group.getPlotterClass() == null ? config.getPlotterClass()
                        : group.getPlotterClass());
                config.setType(group.getType() == null ? config.getType() : group.getType());
                config.setUnitSuffix(group.getUnitSuffix() == null ? config.getUnitSuffix()
                        : group.getUnitSuffix());
                config.addDefaultExtraInfo(new HashMap<>(group.getDefaultExtraInfo()));
